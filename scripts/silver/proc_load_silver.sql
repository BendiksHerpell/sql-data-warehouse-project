/*
Clean Data and load into silver tables ETL process
*/
...
REPLACE(cid,'-','') cid,
CASE 
    WHEN UPPER(REGEXP_REPLACE(cntry, '[^A-Za-z]', '')) = 'DE' THEN 'Germany'
    WHEN UPPER(REGEXP_REPLACE(cntry, '[^A-Za-z]', '')) IN ('US','USA') THEN 'United States'
    WHEN cntry IS NULL 
         OR REGEXP_REPLACE(cntry, '[[:space:]]', '') = '' THEN 'n/a'
    ELSE TRIM(cntry)
  END AS cntry
